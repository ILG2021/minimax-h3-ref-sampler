"""MiniMax H3 runtime patches for Motion Context segment continuity.

Enables interior keyframe time coordinates and keyframe+reference payload
coexistence so this node can pin a previous segment's tail into the next.

Adapted from AIMixer/ComfyUI_MiniMaxH3_Director's original Apache-2.0
implementation. Changes include local ownership markers and integration with
the fixed-audio sliding timeline.
...
log = logging.getLogger("minimax-h3-ref-sampler.h3_context_patches")

# Local markers must not be shared with Director or standalone Motion Context.
CTX_FRAME_KEY = "h3_ref_sampler_context_index"
CTX_AUDIO_END_KEY = "h3_ref_sampler_context_audio_end"
LAYOUT_MARKER = "_h3_ref_sampler_context_layout_patch"
PAYLOAD_MARKER = "_h3_ref_sampler_context_payload_patch"

# Known foreign markers — stand down / refuse rather than double-wrap.
_FOREIGN_LAYOUT_MARKERS = (
    "_h3_director_continuity_layout_patch",
    "_h3_motion_context_layout_patch",
)
_FOREIGN_PAYLOAD_MARKERS = (
    "_h3_director_continuity_payload_patch",
...
def _target_origin(layout) -> float:
    ...
    if kind != "video" or b <= a:
        raise RuntimeError(
            "H3 Ref Sampler Motion Context: expected final layout segment to be target video "
...
def _expected_ref_kinds(block: dict) -> tuple[str, ...]:
    ...
    if kind in ("video", "video_audio"):
        ...
        return ("ref_img",)
    raise RuntimeError(f"H3 Ref Sampler Motion Context: unknown reference kind {kind!r}.")
...
def _ref_segments(layout, refs: list | None) -> dict[int, dict[str, tuple[int, int]]]:
    ...
    if len(wanted) != len(ref_segs):
        raise RuntimeError(
            "H3 Ref Sampler Motion Context: reference segment count mismatch "
    ...
    for (i, kind), (a, b, got) in zip(wanted, ref_segs):
        if got != kind:
            raise RuntimeError(
                f"H3 Ref Sampler Motion Context: reference block {i} expected {kind}, got {got}."
            )
...
def _rewrite_keyframe_times(layout, text_len, latent_t, frame_count, keyframes, refs=None):
    ...
    if offset and any(kf.get(CTX_FRAME_KEY) is None for kf in keyframes):
        raise RuntimeError(
            "H3 Ref Sampler Motion Context: cannot mix unmarked stock keyframes with "
    ...
    if len(cond_spans) != len(keyframes):
        raise RuntimeError(
            f"H3 Ref Sampler Motion Context: expected {len(keyframes)} cond segments, "
...
def _rewrite_audio_timeline(layout, text_len, refs):
    ...
    if len(marked) != 1:
        raise RuntimeError(
            "H3 Ref Sampler Motion Context: audio continuation requires exactly one marked "
    ...
    if blk.get("kind") != "audio":
        raise RuntimeError(
            f"H3 Ref Sampler Motion Context: {CTX_AUDIO_END_KEY} set on non-audio ref "
            f"{blk.get('kind')!r}."
        )
    ...
    if seg is None:
        raise RuntimeError("H3 Ref Sampler Motion Context: marked audio ref produced no layout rows.")
    a, b = seg
    if b - a != 2 * rt:
        raise RuntimeError(
            f"H3 Ref Sampler Motion Context: audio ref rows {b - a} != expected stereo {2 * rt}."
        )
...
setattr(_context_layout_init, LAYOUT_MARKER, True)
...
def _self_test_layout() -> None:
    ...
    if not torch.allclose(stock.position_ids, ours.position_ids, rtol=0, atol=1e-9):
        raise RuntimeError("H3 Ref Sampler Motion Context layout self-test: endpoint mismatch vs stock")
    ...
    if len(times) != 4 or any(times[i] >= times[i + 1] for i in range(3)):
        raise RuntimeError("H3 Ref Sampler Motion Context layout self-test: interior times not increasing")
...
def _classify_layout_owner() -> str | None:
    ...
    if getattr(init, LAYOUT_MARKER, False):
        return "ours"
    if any(getattr(init, marker, False) for marker in _FOREIGN_LAYOUT_MARKERS):
        return "foreign_mc"
    if getattr(init, "__name__", "") in {"_patched_init", "_context_layout_init"}:
...
def ensure_layout_patch() -> bool:
    ...
    if _layout_applied and owner != "ours":
        _layout_applied = False
    if owner == "ours":
    ...
    if owner == "foreign_mc":
        raise RuntimeError(
            "H3 Ref Sampler Motion Context: standalone ComfyUI-H3-Motion-Context (or a fork) "
    ...
    if owner == "foreign_other":
        raise RuntimeError(
            "H3 Ref Sampler Motion Context: another pack already patched MiniMax H3 "
    ...
    if not hasattr(mm, "PackedLayout") or not hasattr(mm, "FRAME_RESCALE"):
        raise RuntimeError("H3 Ref Sampler Motion Context: MiniMax H3 model module incomplete.")
    ...
    try:
        ...
        _layout_orig = None
        raise RuntimeError(
            f"H3 Ref Sampler Motion Context: layout self-test failed ({exc}). "
            "Interior keyframe anchors unavailable."
        ) from exc
    mm.PackedLayout.__init__ = _context_layout_init
    _layout_applied = True
    log.info("H3 Ref Sampler Motion Context: interior keyframe anchors enabled")
    return True


def _context_extra_conds(self, **kwargs):
    ...
    if not isinstance(payload, dict):
        log.warning("H3 Ref Sampler Motion Context: could not reach H3 payload for keyframe+ref merge")
        return out
...
setattr(_context_extra_conds, PAYLOAD_MARKER, True)
...
def _classify_payload_owner() -> str | None:
    ...
    if getattr(fn, PAYLOAD_MARKER, False):
        return "ours"
    if any(getattr(fn, marker, False) for marker in _FOREIGN_PAYLOAD_MARKERS):
        return "foreign_mc"
    if getattr(fn, "__name__", "") in {"_patched_extra_conds", "_context_extra_conds"}:
        return "foreign_other"
...
def ensure_payload_patch() -> bool:
    ...
    if _payload_applied and owner != "ours":
        _payload_applied = False
    if owner == "ours":
    ...
    if owner == "foreign_mc":
        raise RuntimeError(
            "H3 Ref Sampler Motion Context: standalone ComfyUI-H3-Motion-Context already patched "
            "MiniMaxH3.extra_conds. Disable that pack and restart ComfyUI."
        )
    if owner == "foreign_other":
        raise RuntimeError(
            "H3 Ref Sampler Motion Context: another pack already patched MiniMaxH3.extra_conds. "
    ...
    if cls is None or not hasattr(cls, "extra_conds"):
        raise RuntimeError("H3 Ref Sampler Motion Context: MiniMaxH3.extra_conds not found.")
    _payload_orig = cls.extra_conds
    cls.extra_conds = _context_extra_conds
    _payload_applied = True
    log.info("H3 Ref Sampler Motion Context: keyframe/ref coexistence enabled")
    return True


def ensure_context_patches() -> None:
    """Preflight both global entry points, then install them as one operation."""
    layout_owner = _classify_layout_owner()
    payload_owner = _classify_payload_owner()
    if layout_owner not in (None, "ours"):
        label = (
            "standalone H3 Motion Context"
            if layout_owner == "foreign_mc"
            else "another custom node"
        )
        raise RuntimeError(
            "H3 Ref Sampler Motion Context cannot start because "
            f"{label} already owns PackedLayout.__init__."
        )
    if payload_owner not in (None, "ours"):
        label = (
            "standalone H3 Motion Context"
            if payload_owner == "foreign_mc"
            else "another custom node"
        )
        raise RuntimeError(
            "H3 Ref Sampler Motion Context cannot start because "
            f"{label} already owns MiniMaxH3.extra_conds."
        )
    ensure_layout_patch()
